...
const INVERT_WINDOW_COVERINGS_TILT_DIRECTION = 'invertWindowCoveringsTiltDirection';
...
module.exports = {
  ...
  setParserV2(value, options) {
    const duration = options.hasOwnProperty('duration')
      ...
      : FACTORY_DEFAULT_DIMMING_DURATION_V2;
  ...
  setParserV4(value, options) {
    ...
    const duration = options.hasOwnProperty('duration')
      ? Buffer.from([util.calculateZwaveDimDuration(options.duration)])
      : FACTORY_DEFAULT_DIMMING_DURATION_V4;
    if (this.hasCapability('onoff')) this._setCapabilityValueSafe('onoff', value > 0);
  ...
  reportParserV4(report) {
    ...
    if (report && report.hasOwnProperty('Current Value (Raw)')) {
      ...
      return invertDirection
        ? (99 - report['Current Value (Raw)'][0]) / 99
        : report['Current Value (Raw)'][0] / 99;
    }
    return null;
  },
};
